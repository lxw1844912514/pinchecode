...
use Addons\pinche\Model\OpinionModel;
use Mp\Controller\MobileBaseController;
use Addons\pinche\Model\BsUserModel;
...
class MobileController extends MobileBaseController
{
    ...
    {
        if (IS_POST) {
            $User = new BsUserModel;
            $username = I('name');
            $password = I('password');
            if (empty($username)) {
                $this->ajaxReturn(array('status' => false, 'data' => create_addon_url('login'), 'error' => '用户名不能为空'));
                //$this->error('用户名不能为空');
            }
            if (empty($password)) {
                $this->ajaxReturn(array('status' => false, 'data' => create_addon_url('login'), 'error' => '密码不能为空'));
                //$this->error('密码不能为空');
            }
            if (!$User->is_user_exists($username)) {
                $this->ajaxReturn(array('status' => false, 'data' => create_addon_url('login'), 'error' => '用户不存在'));
                //$this->error('用户不存在');
            }
            if (!$User->auto_login($username, $password)) {
                $this->ajaxReturn(array('status' => false, 'data' => create_addon_url('login'), 'error' => '登录密码错误'));
            }
            $this->ajaxReturn(array('status' => true, 'data' => create_addon_url('index'), 'error' => '登录成功'));
        } else {
            $this->display();
        }
    }
    ...
    public function register()
    {
        if (IS_POST) {
            ...
            if (!$User->create()) {
                $json['status'] = false;
                $json['data'] = create_addon_url('register');
                $json['error'] = $User->getError();
                ...
                $user_id = M('BsUser')->add($data);
                if ($User->auto_login(I('name'), I('password'))) {
                    $json['status'] = true;
                    $json['data'] = create_addon_url('login');
                    $json['error'] = '用户注册成功并已自动登录';
                    $this->ajaxReturn($json);
                    exit();
                } else {
                    $json['status'] = true;
    ...
    /**退出登录
     * User: lxw
     * Date: 2021-04-11 12:50
     */
    public function login_out()
    {
        $User = new BsUserModel;
        $User->login_out();
        $url = create_addon_url('login');
        header("location: $url");
        exit();
    }
    ...
    public function opinion()
    {
        if (IS_POST) {
            ...
            if (!$Opinion->create()) {
                $json['status'] = false;
                $json['data'] = create_addon_url('opinion');
                $json['error'] = $Opinion->getError();
                $this->ajaxReturn($json);
            } else {
                $data['name'] = session('username_client');
                $data['user_id'] = session('user_info_client')['id'];
                $data['created_at'] = time();
                $data['content'] = I('content');
                $opinionId = M('opinion')->add($data);
                if ($opinionId) {
                    $json['status'] = true;
                    $json['data'] = create_addon_url('index');
                    $json['error'] = '发表成功';
                    $this->ajaxReturn($json);
                    exit();
                } else {
    ...
    public function index($type = '', $go = '')
    {
        ...
        $where = (isset($_GET['ffrom'])&&isset($_GET['tto']))   ?array('from'=>$_GET['ffrom'],'to'=>$_GET['tto']):array();
        //拼车列表
        $lists = M('bs_pinche')->where($where)->where('status', 1)->order('pubtime desc')->select();
    ...
    public function add()
    {
        ...
        $this->assign('type', session('user_info_client')['type']);
...
//            查询 改用户今日发布了多少条
        ...
        if (IS_POST) {
            ...
            $data['nickname'] = $userinfo['nickname'] ?? session('username_client');
            $data['pubtime'] = time();
            $data['status'] = 1;
            ...
            $data['contact'] = $_POST['Contact'] ? $_POST['Contact'] : session('username');
...
//            time is end?
    ...
    public function detail($id = null)
    {
        ...
        $car_types = M('car')->field(array('name', 'car_type'))->where('type', $car_type)->find();
        $carTypeArr = array(1 => '小轿车', 2 => 'SUV', 3 => '微面', 4 => '货车');

        $info['CarType'] = $car_types['name'] . $carTypeArr[$car_types['car_type']];
        $jssdk_sign = get_jssdk_sign_package();
        $this->assign('config', $jssdk_sign);
...
//        $where['nickname'] = session('username');
    ...
    public function del($id = null)
    {
        ...
        if ($pinche->where($where)->save(array('status' => 0))) {
            $data['status'] = true;
            $data['data'] = null;
            $data['error'] = '';
            $this->ajaxReturn($data);
